...
{
    #region Variables
    public KeyCode EquipmentOpen = KeyCode.X;
    [Header("Inventory")]
    public bool showInv;
    ...
    {
        // Call the rigidbody
        // If the Item is Dragged out, spawn the mesh randomly, and give it gravity
        droppedItem = draggedItem.ITEMMESH;
        droppedItem = Instantiate(droppedItem, new Vector3(Random.Range(1f, -4f), Random.Range(0f, 2f), Random.Range(1f, 6f)), Quaternion.identity);
    ...
    private void Start()
    {
        ...
        inventorySize = new Rect(scr.x, scr.y, scr.x * 1, scr.y * 4.5f);
    ...
    private void Update()
    {
        if (Input.GetKeyDown(EquipmentOpen))
        {
            ToggleInv();
        }
    ...
    private void OnGUI()
    {
        ...
        if (showInv)
        {
            // EQUIPMENT FIX: 2 so equipment have another ID
            inventorySize = ClampToScreen(GUI.Window(2, inventorySize, InventoryDrag, "Equipment"));
